...
combine_query_boolean.gold <- function(db_query,
  ...
  ### Reduce to variables of interest
  if (query_type %in% c("med", "drug")){
    cohort_qry <- cohort_qry[,c("patid", "indexdt", "eventdate")]
  }

  ### Remove values outside of specified time range
  cohort_qry <- cohort_qry[eventdate <= indexdt + time_post & eventdate > indexdt - time_prev]
...
#' Combine a HES database query with a cohort returning a 0/1 vector depending on whether each individual has a recorded code of interest.
#'
#' @description
#' An S3 method that can be used on database queries from Aurum extracts. Combine a database query with a cohort returning a 0/1 vector depending on whether each individual has a recorded code of interest.
#' `cohort` must contain variables `patid` and `indexdt`. The database query will be merged with the cohort by variable \code{patid}.
...
combine_query_boolean.hes_primary <- function(db_query,
  ...
  ### Reduce to variables of interest
  cohort_qry <- cohort_qry[,c("patid", "indexdt", "admidate")]

  ### Remove values outside of specified time range
  cohort_qry <- cohort_qry[admidate <= indexdt + time_post & admidate > indexdt - time_prev]
...
combine_query.gold <- function(db_query,
  ...
  if (query_type == "test"){
    ### If values are missing, < lower_bound or > upper_bound then delete
    if (value_na_rm == TRUE){
      cohort_qry <- cohort_qry[!is.na(value) & data2 > lower_bound & data2 < upper_bound]
    }
  }
  ...
  ### Reduce to variables of interest
  if (reduce_output == TRUE){
    if (query_type %in% c("med")){
      cohort_qry <- cohort_qry[,c("patid", "medcode", "eventdate")]
    } else if (query_type == "test"){
      cohort_qry <- cohort_qry[,c("patid", "medcode", "eventdate", "data2", "data3", "data5", "data6")]
      ### Rename test data variables for interpretability
      cohort_qry <- dplyr::rename(cohort_qry, value = data2, lookup_sum = data3, numrangelow = data5, numrangehigh = data6)
    } else if (query_type == "drug"){
      cohort_qry <- cohort_qry[,c("patid", "prodcode", "eventdate")]
    }
  }
...
#' Combine a HES Admitted Patient Care database query with a cohort.
#'
#' @description
#' An S3 method that can be used on database queries from Aurum extracts.
#' Combine a database query with a cohort, only retaining observations between `time_prev` days prior to \code{indexdt}, and `time_post` days after
...
combine_query.hes_primary <- function(db_query,
  ...
  ### Reduce to variables of interest
  if (reduce_output == TRUE){
    cohort_qry <- cohort_qry[,c("patid", "admidate")]
  }

  ### Group by patid and admidate and keep the most recent 'numobs' number of observations
  if (earliest_values == FALSE){
    cohort_qry <- dplyr::group_by(cohort_qry, patid) |>
      dplyr::filter(dplyr::row_number(dplyr::desc(admidate)) <= numobs) |>
      data.table::as.data.table()
  } else if (earliest_values == TRUE){
    cohort_qry <- dplyr::group_by(cohort_qry, patid) |>
      dplyr::filter(dplyr::row_number(admidate) <= numobs) |>
      data.table::as.data.table()
  }
...
#' Combine a ONS death dataset query with a cohort.
#'
#' @description
#' An S3 method that can be used on database queries from Aurum extracts.
#' Combine a database query with a cohort, only retaining observations between `time_prev` days prior to \code{indexdt}, and `time_post` days after
...
combine_query.ons_death <- function(db_query,
  ...
  ### Remove values outside of specified time range
  cohort_qry <- cohort_qry[dod <= indexdt + time_post & dod > indexdt - time_prev]

  ### Reduce to variables of interest
  if (reduce_output == TRUE){
    cohort_qry <- cohort_qry[,c("patid", "dod")]
  }

  ### Group by patid and dod and keep the most recent 'numobs' number of observations
  if (earliest_values == FALSE){
    cohort_qry <- dplyr::group_by(cohort_qry, patid) |>
      dplyr::filter(dplyr::row_number(dplyr::desc(dod)) <= numobs) |>
      data.table::as.data.table()
  } else if (earliest_values == TRUE){
    cohort_qry <- dplyr::group_by(cohort_qry, patid) |>
      dplyr::filter(dplyr::row_number(dod) <= numobs) |>
      data.table::as.data.table()
  }
